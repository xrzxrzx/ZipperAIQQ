...
| 你想做的事 | 动哪里 | 注意 |
|---|---|---|
| **加一个服务 / 接口实现** | **在它所属模块的 `AddXxx` 扩展方法里注册** —— 别写进 `Program.cs` | ⚠️ `Program.cs` 只负责**把各模块串起来**（目标 ≤ 20 行）；细节下沉到模块 |
| **需要"启动时连什么"** | 实现 `IHostedService`（异步运行期任务）或 `IStartupCheck`（同步启动前自检） | ⚠️ **注册阶段（`AddXxx` 里）不许做任何 I/O** —— 那时还没有可用的容器 |
...
> ⚠️ **一条硬规则**：每个 `AddXxx` **必须能独立调用** —— 不许依赖"别的模块的 `AddXxx` 已经跑过"（`Core` 除外）。
> **验收方法**：把 `Program.cs` 里 `AddXxx` 的顺序随便打乱，程序仍应正常工作。做不到就说明有隐式依赖。
...
| **组合根（Composition Root）** | **唯一**装配依赖注入容器的地方。本项目只有两个：服务端的 `Program.cs` 与客户端的 `App.xaml.cs` |
| **`AddXxx` 扩展方法** | 每个模块提供自己的注册扩展（`AddZipperAgent` 等），组合根只负责把它们串起来 |
